...
void  cast_horizontal_inter_ray(float angle, int x)
{
	...
	// if (map.ray[x].ray_up)
		...
		while(nextx >= 0 && nextx <= map.el.res_x  && nexty >=  0 && nexty <= map.el.res_y )
		{
			...
			if (is_wall2(xtocheck,ytocheck,'2'))
			{
				map.ray[x].issprite = 1;
				horzhitsprites_x = xtocheck;
				horzhitsprites_y = ytocheck;
				is_vertical = 0;
			//render_sprites(xtocheck,ytocheck,x);
				//break;
			}
	...
	float xintercept;
	 ...
	 //  nextx--;
		while(nextx >= 0 && nextx <=map.el.res_x && nexty >=0 && nexty <= map.el.res_y )
		{
			...
			{
				map.ray[x].issprite = 1;
				verhitsprites_x = xtocheck;
				verhitsprites_y = ytocheck;
				is_vertical = 1;
			//render_sprites(verhitsprites_x,verhitsprites_y,x);
				//break;
			}
			if (is_wall2(xtocheck,ytocheck,'1'))
			{
...
/*void    project_wall()
{
		...
		if (map.ray[x].wasvertical)
				 ...
				 texx = map.ray[x].wallhity / map.wall_width;
			...
			if(map.ray[x].ray_up && !map.ray[x].wasvertical)
			{
				...
				while (y < wallbottomPixel)
				{
				 int distancefromtop = y + (wallstripheight / 2) - (map.el.res_y / 2);
						texy = distancefromtop * (float)(map.tex1[1].texheight / wallstripheight);
					//texy = (y - ((map.el.res_y / 2)- (wallstripheight/2))) * ((float)map.tex1[1].texheight / wallstripheight);
						img.data[(map.el.res_x * y) + x] = map.tex.color_s[(map.tex1[1].texwidth * (int)texy) + (int)texx];
						y++;
					}
			}
			////south
			if(map.ray[x].ray_down && !map.ray[x].wasvertical)
			{
				...
				//		texy = distancefromtop *  (float)(map.tex.texheight / wallstripheight);
						texy = (y - ((map.el.res_y / 2)- (wallstripheight/2))) * ((float)map.tex1[0].texheight / wallstripheight);
						img.data[(map.el.res_x * y) + x] = map.tex.color_n[(map.tex1[0].texwidth * (int)texy) + (int)texx];
						y++;
					}
			}
			if(map.ray[x].ray_right && map.ray[x].wasvertical)
			{
				...
				{
				 int distancefromtop = y + (wallstripheight / 2) - (map.el.res_y / 2);
						texy = distancefromtop *  (float)(map.tex1[3].texheight / wallstripheight);
						img.data[(map.el.res_x * y) + x] = map.tex.color_w[(map.tex1[3].texwidth * (int)texy) + (int)texx];
						y++;
					}
			}
			if(map.ray[x].ray_left && map.ray[x].wasvertical)
			{
				...
				{
				 int distancefromtop = y + (wallstripheight / 2) - (map.el.res_y / 2);
						texy = distancefromtop * (float)(map.tex1[2].texheight / wallstripheight);
						img.data[(map.el.res_x * y) + x] = map.tex.color_e[(map.tex1[2].texwidth * (int)texy) + (int)texx];
						y++;
					}
			}
			...
			{
				img.data[u * map.el.res_x + x] =  map.el.f_color_hex;
				u++;
			}
			///render_w_s(x);
			 // draw_sprit(x);
